...
- (void)viewDidLoad {
    ...
    self.preferences = [[HBPreferences alloc] initWithIdentifier:@"love.litten.astolfopreferences"];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/Astolfo.disabled"]) {
        [[self enableSwitch] setEnabled:NO];

        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Astolfo" message:@"Astolfo has detected that you have disabled it with iCleaner Pro, here are some quick actions you can perform" preferredStyle:UIAlertControllerStyleAlert];
        ...
        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Astolfo" message:@"Astolfo has detected that you have DayNightSwitch installed, which causes issues with Astolfo's preferences\n\n To continue, please disable DayNightSwitch with iCleaner Pro or uninstall it temporarily" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)resetPrompt {
    ...
    UIAlertController* resetAlert = [UIAlertController alertControllerWithTitle:@"Astolfo" message:@"Do you really want to reset your preferences?" preferredStyle:UIAlertControllerStyleActionSheet];
...
- (void)respring {
    ...
    [UIView animateWithDuration:1 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/shuffle.dylib"])
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Astolfo"]];
        else
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Tweaks&path=Astolfo"]];
    }];
